...
-   **Key Features**:
    ...
    -   **Item Roll Templates (AA V2 only)**: Admins create lightweight roll templates (name + roll settings only) in the Item Templates view. Inside the AAv2 admin Item create/edit dialog, a collapsible "Roll Templates" panel exposes a checkbox list of every template; ticking a template copies its rolls onto the item with `fromTemplateRollId` pointers and links the item via the `item_template_links` join table. Subsequent edits to a template's rolls propagate live to every linked item (including character-owned copies) through the existing roll propagation pipeline. Inherited rolls are surfaced read-only in `RollEntriesEditor` (no edit/move/duplicate/delete buttons).
