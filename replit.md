...
- **Key Features**:
    ...
    - **Sandbox System**: FoundryVTT-style custom system builder. Templates define properties (text, number, checkbox, textarea, select) with a visual drag-and-drop canvas editor. Each property has configurable position (x/y), size (width/height), label font size (8-24px), value font size (8-24px), and label position (top/left/hidden). Grid snapping (10px) for alignment. Template settings control default sheet width/height and resize permission. Actors inherit template layouts and render properties with absolute positioning matching the GM's design. Features a non-blurring 320px side panel, floating/draggable editors, hierarchical folder organization, and GM-only authorization. Property data stored in template/actor `data` JSON columns. Legacy properties without layout fields receive runtime defaults.
