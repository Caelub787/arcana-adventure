...
- **Key Features**:
    ...
    - **Sandbox System**: FoundryVTT-style Dynamic System Builder. GMs create custom game systems with a visual drag-and-drop canvas editor. Properties can be placed freely anywhere — directly on the canvas root or inside containers.
      ...
      - **Tabs**: Tab nodes control visibility of child nodes. They have `behaviorConfig.tabConfig` with `tabPosition` (top/bottom/left/right) and `activeTabId`. Can be nested at any depth.
      ...
      - **Property Types**: text, number, boolean (checkbox), resource (current/max dual inputs), list (select dropdown), formula (read-only computed), pfp (profile picture).
      - **Default Template Structure**: New templates start with a blank canvas containing PFP and Name properties placed directly on canvas root (no mandatory sections).
