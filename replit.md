...
- **Species/Race System**: Database-driven species management via Admin Settings. Supports custom species with auto-filled attributes (lifespan, speed, flySpeed, size, naturalArmor, sizeBonus, startingHP, startingMaxHP, hpPerLevel, startingEnergy, startingMaxEnergy, featTree, defaultImage). Human species seeded as baseline. Species can have a default token image that is used when creating character tokens if the character has no portrait.
