...
-   **Key Features**:
    ...
    -   **World Builder Tabs**: Encyclopedia section supports browser-like tabs. Clicking an article from the main encyclopedia grid opens it in a new tab; clicking a link within an article (sidebar navigation) opens in the current tab. Tab bar with Home button and close buttons. State: `wbTabs` array and `activeWbTabId` in WorldBuilderContent.
