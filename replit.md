...
- **Key Features**:
    ...
    - **Sandbox System**: Rules-agnostic Dynamic System Builder. GMs design, structure, and play any VTTRPG system or create entirely new ones. No hardcoded RPG assumptions — every stat, rule, interaction, and layout is user-defined. Separates structure (Template Sheets) from gameplay (Actor Sheets). Properties can be placed freely anywhere — directly on the canvas root or inside containers.
      - **Vision/Roadmap**: Full formula/expression engine (math, conditionals, cross-actor references), display rules (conditional visibility), actor interactions (embedded actors, actor references), decoupled tab buttons (placeable anywhere, multiple buttons per tab), resource enhancements (color thresholds, gradient blending), label property (rich text with HTML/inline references), explicit Builder/Play mode separation, per-property permissions.
      ...
      - **Tabs**: Tab nodes control visibility of child nodes. They have `behaviorConfig.tabConfig` with `tabPosition`, `activeTabId`, `tabLayout` (top/left/right for button placement), `tabButtonSize` (small/medium/large), and `tabIcons` (per-child-panel icon/emoji/image with optional showName flag). Tab icons/images replace tab names by default; showName flag enables displaying both. Tab creation auto-generates 2 child panels. Tab bar supports: click to switch, double-click to rename, right-click for context menu (rename/delete/move/set icon/upload image/toggle name/tab size), and "+" button to add new tab pages. Can be nested at any depth.
      ...
      - **Properties**: Properties use `parentId` (null = canvas root, string = inside a layout node). Any layout node type can host properties. Each property has `id`, `key`, `type`, `parentId`, `defaultValue`, and nested `metadata` containing `label`, `tooltip`, `uiConfig` (x/y/width/height/labelFontSize/valueFontSize/labelPosition), `style`, `resourceConfig`, `calculationExpression`, `visibilityExpression`.
      - **Property Types**: text, number, boolean (checkbox), resource (current/max dual inputs), list (select dropdown with options from comma-separated default value), textarea (multi-line text field sized per template), pfp (profile picture with image upload), label (rich text with HTML and {{key}} inline references).
      - **Type-Specific Settings**: Each property type has unique settings in the property settings panel: text/textarea get placeholder, number gets min/max/step, resource gets show-bar/bar-color/allowOverMax/colorThresholds/useGradient, list uses default value for comma-separated options, pfp sets 100x100 with hidden label, label gets textarea for rich content with {{key}} syntax. Property type can be changed from the settings panel.
      - **Expression Engine**: Safe recursive descent parser at `client/src/components/sandbox/expressionEngine.ts`. Supports arithmetic (+,-,*,/,%), comparisons (>,<,>=,<=,==,!=), boolean logic (&&,||,!), ternary (?:), functions (min,max,clamp,floor,ceil,round,abs), and property key references. Resource properties support `.current`/`.max` suffixes. No eval/Function — max 1000 chars, depth 50.
      - **Calculation Formulas**: Properties can have a `calculationExpression` that computes their value from other properties. Formula-driven values are read-only in actor view with amber "fx" badge indicator.
      - **Display Rules (Conditional Visibility)**: Properties can have a `visibilityExpression` that determines whether they're shown. Expression evaluates to boolean; property hidden when false. Applied in both actor and mobile views.
      - **Resource Enhancements**: Color thresholds (percent-based color stops), gradient blending between threshold colors, dynamic bar coloring based on current/max ratio. Configurable via property settings.
