...
-   **Key Features**:
    ...
    -   **AA V2 System**: Alternate campaign system (`aa-v2` slug) with mana resource (mana/maxMana on characters), class-based progression with class skill points (3 per level, 5 on levels divisible by 3), and mana costs on rolls/spells. Mana bar on character sheet matches HP/energy bar design (violet). Token mana bar is purple/pink (fuchsia-500). Full classes system with admin class CRUD, per-class skill tree editor (draggable nodes, connections, tier-based styling, effects), character sheet Classes tab (add/remove classes, level up individual classes, view/unlock skill tree nodes). Class points formula: `2 + classLevel + (2 * Math.floor(classLevel / 3))`. Node effects: hp_bonus, energy_increase, mana_increase, attribute_bonus, skill_bonus. WebSocket real-time sync for class changes.
