...
-   **Key Features**:
    ...
    -   **World Builder Tabs**: True browser-style tab system across the entire World Builder. Any section (Home, Encyclopedia, Maps, Timeline, Calendar, Graph) or individual article can be opened as its own tab. All tabs stay mounted in the DOM (display:none/block) for instant switching. Tab types: `home | encyclopedia | article | maps | timeline | calendar | graph`. State: `wbTabs: WbTab[]` and `activeWbTabId` in WorldBuilderContent. Section nav buttons create/switch tabs. Article tabs have a sidebar article list. Per-tab state for maps (editingMapId, creatingMap) and timeline (selectedTimelineId).
