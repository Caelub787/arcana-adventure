...
- **Fog Opacity Control**: Fog opacity slider now controls the actual darkness level outside token vision. Previously hardcoded to 100% black.
- **Explored Memory System**: Toggle in GM Fog of War panel. When ON, unexplored areas are 100% black, explored-but-not-visible areas use Explored Dimness opacity, currently visible areas are clear. Client-side tracking using grid cell Set, reset on scene change or toggle off. Two-mask SVG rendering approach.
- **Combined Vision Export**: `onVisionPolygonsChange` now exports both base vision AND light vision polygons, so tokens in lit areas are visible to players via point-in-polygon filtering.
- **Light Vision Fix**: Rewrote `calculateVisionInLight()` with proper angular span (`Math.asin`), tangent angles, filtered blocking segment angles, and correct ray-circle intersection (entry vs exit point). Finer sweep resolution (PI/72).
- **Wall Corner Vision Fix**: Fixed bug where wall corners blocked vision down straight hallways when token was on the same row. Root cause: rays hitting wall endpoints (u=0 or u=1) were treated as blocks even when the wall extended perpendicular/backward from the ray direction. Fix: `castRay` now checks the dot product of the wall body direction and ray direction at endpoint hits — if the wall extends sideways or backward (dot ≤ 0), the endpoint hit is skipped, allowing vision to pass the corner. Also increased angular EPSILON from 0.0005 to 0.001 for better spatial resolution at distance.
- **Fog Performance**: Replaced SVG mask-based fog rendering with even-odd fill-rule paths (eliminates 20000x20000 mask buffer compositing). Changed CSS from `willChange: 'contents'` to `contain: 'layout style paint'`. Cached light illumination polygons (pre-computed once when walls/lights change, reused across token calculations).
- **Default Vision Distances**: Changed from 120ft/60ft to 60ft day/30ft night across systemSpecies, campaignSpecies, and characters (schema defaults + backfilled existing records).
- **Trait Vision Modifiers**: Batch API endpoint (`/api/campaigns/:campaignId/trait-vision-modifiers`) returns day/night vision bonuses per character from traits. FogOfWarOverlay fetches and applies these modifiers to per-token vision distance calculation with 30s cache.
- **Vision Zones System**: `scene_vision_zones` table with API. GM draws freeform polygons on the map via FogToolsPanel zone drawing mode. Zones can be "indoor" (forces night vision distances) or "outdoor" (forces day vision distances), overriding the scene's day/night setting per-token. Last-drawn zone has highest priority when zones overlap. Purple styling for indoor, amber for outdoor. Zone list with delete in panel. `isTokenIndoor()` helper uses point-in-polygon to determine per-token effective day/night.