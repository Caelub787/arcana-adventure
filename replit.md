...
- **Key Features**:
    ...
    - **Sandbox System**: FoundryVTT-style custom system builder. Templates define properties (text, number, checkbox, textarea, select, panel, tab, pfp) with a visual drag-and-drop canvas editor. Each property has configurable position (x/y), size (width/height), label font size (8-24px), value font size (8-24px), and label position (top/left/hidden). Grid snapping (10px) for alignment. Template settings control default sheet width/height and resize permission. Actors inherit template layouts and render properties with absolute positioning matching the GM's design. Features a non-blurring 320px side panel, floating/draggable editors, hierarchical folder organization, and GM-only authorization. Property data stored in template/actor `data` JSON columns. Legacy properties without layout fields receive runtime defaults.
      - **Default Template Structure**: New templates auto-generate with Header panel (top, containing PFP and Name properties) and Body panel (below header).
      ...
      - **Panel Containers**: Panel property type acts as a true styled container grouping child properties. Children positioned relative to panel's content area with overflow clipping. Children assigned via "Parent Container" dropdown in property settings.
      - **Tab Containers**: Tab property type with switchable named tabs, each with its own canvas area. Nestable (tabs in tabs, panels in tabs). Tab management (add/rename/delete) in property settings. Active tab tracked per container. Tab switching works in both canvas editor and actor sheets. Tabs support icon/image URLs as alternatives to text labels. Tab position configurable (top/left). Tabs support background colors/gradients like panels.
      - **Container Add Buttons**: Panel, tab, and header/body containers have + buttons to add properties directly within them, auto-assigning parentId and tabId.
      - **Drag-and-Drop Reparenting**: Properties can be dragged between containers (panels/tabs) to change their parent. Drop target containers highlight with a green ring indicator. Dragging a property outside its current parent unparents it to root level.
