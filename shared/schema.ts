...
export const users = pgTable("users", {
  ...
  theme: text("theme"), // Unused - the app now has a single fixed theme (Arcane Library), kept for backward compat
...
export const characters = pgTable("characters", {
  ...
  // C.A. only: freeform wounds pinned at a (x, y) position on the body
  // diagram, replacing HP entirely for this system. Purely descriptive/
  // GM-discretion — no mechanical stat derivation. See shared/ca.ts for the
  // CAWound shape.
  caWounds: jsonb("ca_wounds").$type<CAWound[]>().notNull().default(sql`'[]'::jsonb`),
  // C.A. only: which body diagram renders behind the wound markers. Defaults
...
export type InsertFreeHotbarEntry = z.infer<typeof insertFreeHotbarEntrySchema>;
export type FreeHotbarEntry = typeof freeHotbarEntries.$inferSelect;
