...
export interface V3CastTechnique {
  ...
  // from inventory server-side when the technique fires (see the
  // `POST /api/v3/techniques/:id/use` route — the client no longer consumes).
...
export function castV3Technique(
  ...
  // When the server is the authority for resources (skipEnergy), it has already
  // verified sufficiency and deducted energy — the client only rolls for display.
  if (!opts?.skipEnergy && energyCost > 0 && have < energyCost) {
