...
export function BattleMap({ tokens, onMoveToken, onTokenClick, role, gridSize }: BattleMapProps) {
  // Pan state
  const [pan, setPan] = useState({ x: 0, y: 0 });
  const constraintsRef = React.useRef(null);

  const handleDragEnd = (e: any, info: any, token: Token) => {
    // Snap to grid logic using the dynamic gridSize
    // Note: Framer motion drag is relative to the parent (the panned map layer)
    // We need to account for the pan offset if we are using screen coordinates, 
    // but 'info.offset' is relative to the element's start position.
    // 'token.x' and 'token.y' are absolute positions within the map layer.
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-black rounded-lg border border-white/10 shadow-inner group" ref={constraintsRef}>
      ...
      <motion.div 
        ...
        // We use a large negative margin to center the "origin" visually if we want, but default top-left is fine for now
        style={{ top: 0, left: 0 }} 
      >
        {/* Map Background */}
        <div 
        ...
        {tokens.map((token) => (
          <motion.div
            ...
            dragMomentum={false}
            dragElastic={0}
            // Important: Stop propagation so dragging a token doesn't drag the map
