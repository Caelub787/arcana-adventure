...
import { X, FileText, Grid3X3, Network, History, Map as MapIcon, BookOpen, Table } from "lucide-react";
...
export interface OpenNote {
  noteId: string;
  title: string;
  type?: "markdown" | "canvas" | "sheet" | "graph" | "timeline" | "scene" | "book";
}
            ...
            return (
              ...
              >
                {note.type === "graph" ? (
                  ...
                  <Table className={`flex-shrink-0 ${compact ? 'h-3 w-3' : 'h-3.5 w-3.5'} ${isActive ? 'text-teal-400' : 'text-stone-500'}`} />
                ) : note.type === "book" ? (
