...
**Compositor pan is invisible to JS:** iOS keyboard "shove/black flash" is a compositor pan (changes visualViewport.offsetTop, NOT window.scrollY) — post-hoc clamping can never remove the painted flash. Only cure: PRE-reveal the field above where the keyboard will be, synchronously on focusin (touch devices only), using the last observed keyboard inset when the viewport hasn't resized yet.

**Never fight iOS caret pans:** with the keyboard OPEN, WebKit re-pans the window on every keystroke to keep the caret visible; any JS that scrolls back while typing creates a per-keystroke tug-of-war that paints a black flash (dialog backdrops show through). Rule: while keyboard is open, allow window scroll up to the keyboard inset; only snap back when the keyboard closes/focus leaves. Pre-reveal on focusin (one-shot) is fine; continuous reveal/clamp during typing is not.

**Overlay/content must get DISTINCT z slots:** the shadcn dialog & alert-dialog wrappers assign z imperatively via bringFloatingPanelToFront(); giving the backdrop and the content the SAME z lets iOS flip the two GPU layers during per-keystroke re-composites (black backdrop flashes over the form). Always bump the counter twice — overlay first, content strictly above.

**Never acquire shared z in a ref callback without a mount guard:** Radix re-attaches composed refs on EVERY re-render, so a z-acquiring ref callback runs per keystroke — churning the shared counter and rewriting fixed layers' z, which the compositor repaints as a dark-backdrop flash over the form on all devices. Guard with el.dataset flag (acquire once per mount). Backdrops should be plain static rgba, no fade animation, and strictly below content z (never tied).
